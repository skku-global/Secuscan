...
| `test_tier1_false_positives.py` | that every Tier 1 check returns zero findings against realistic, clean, production-grade security configurations: strict CSP directives, modern HSTS preload policies, safe wildcard CORS, mixed-content-clean pages, modern TLS cipher suites, properly flagged cookies (`Secure`, `HttpOnly`, `SameSite`), and well-formed SPF / DMARC records |
| `test_finding.py` | the finding builder contracts, tier propagation, fallback descriptions and remediations, CVSS score boundaries per severity level, OWASP Top 10 category tagging, and JSON dictionary serialization |
| `test_engine.py` | the overall security scoring calculation across severity distributions, score floors at 0, check crash containment (`_crash_finding` recording an INFO/engine error finding rather than failing the scan), and full scan orchestration with mock checks |
| `test_plan_limits.py` | billing period calculation (rolling 30-day window for Free plans, period-end relative for active subscriptions), scan counts and distinct site counts per period, target host tracking, and pre-scan quota enforcement returning HTTP 403 on limit breaches |
| `test_account_deletion_export.py` | GDPR data export stripping secret hashes (`passwordHash`, `totpSecret`, `tempTotpSecret`, `recoveryCodeHashes`) as a JSON attachment download, and step-up password authenticated account deletion cascading across users, sessions, scans, and credentials while anonymizing orders for tax and accounting audit compliance |
